...
def SenSpeForAUC(fpr_tpr_tuple):
    """
    Get sen spe for AUC.
    Sen = tpr
    Spe = 1 - fpr

    :param fpr_tpr_tuple: tuple, the output of sklearn.metrics.roc_curve
    :return: sen, spe
    """
    # fpr, tpr
    fpr = fpr_tpr_tuple[0]  # fpr values
    tpr = fpr_tpr_tuple[1]  # tpr values
    # sum, diff
    sen_spe_sum = tpr - fpr + 1
    sen_spe_diff = abs(tpr + fpr - 1)
    # index
    max_value = max(sen_spe_sum)
    max_index = np.where(sen_spe_sum == max_value)[0]
    target_index = max_index[np.argmin(sen_spe_diff[max_index])]

    sen = tpr[target_index]
    spe = 1 - fpr[target_index]
