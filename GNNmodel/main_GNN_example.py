...
list_record_sen_auc_train, list_record_spe_auc_train = [], []
list_record_sen_auc_test, list_record_spe_auc_test = [], []
                ...
                for learning_rate in list_lr:
                    for layer_size1 in list_lz1:
                        for layer_size2 in list_lz2:
                            ...
                            train_sen_auc_epochs_list, train_spe_auc_epochs_list = [], []
                            # Test Sample Info
                            test_auc_epochs_list = []
                            test_sen_auc_epochs_list, test_spe_auc_epochs_list = [], []
                            ...
                            for epoch in range(MAX_EPOCH_NUM):
                                ...
                                train_sen_auc_epoch, train_spe_auc_epoch = \
                                    Functions_PyG.SenSpeForAUC(metric.roc_curve(data_network_train.y.numpy(), train_prob_epoch))
                                ...
                                if (epoch + 1) in list_epoch_num:
                                    ...
                                    test_sen_auc_epoch, test_spe_auc_epoch = \
                                        Functions_PyG.SenSpeForAUC(metric.roc_curve(labels_test, probs_test_temp))
                                    ...
                                    train_sen_auc_epochs_list.append(train_sen_auc_epoch)
                                    train_spe_auc_epochs_list.append(train_spe_auc_epoch)

                                    # Results Record ## Test
                                    test_auc_epochs_list.append(test_auc_epoch)
                                    test_sen_auc_epochs_list.append(test_sen_auc_epoch)
                                    test_spe_auc_epochs_list.append(test_spe_auc_epoch)
                                    ...
                                    list_record_sen_auc_train.append(train_sen_auc_epochs_list[-1])
                                    list_record_spe_auc_train.append(train_spe_auc_epochs_list[-1])
                                    list_record_auc_test.append(test_auc_epochs_list[-1])
                                    list_record_sen_auc_test.append(test_sen_auc_epochs_list[-1])
                                    list_record_spe_auc_test.append(test_spe_auc_epochs_list[-1])
...
df_Summary = pd.DataFrame({'record_str_k_value': list_record_k_value, 'record_str_knn_type': list_record_str_knn_type,
                           ...
                           'record_auc_train': list_record_auc_train,
                           'record_sen_auc_train': list_record_sen_auc_train,
                           'record_spe_auc_train': list_record_spe_auc_train,
                           'record_auc_test': list_record_auc_test,
                           'record_sen_auc_test': list_record_sen_auc_test,
                           'record_spe_auc_test': list_record_spe_auc_test})
